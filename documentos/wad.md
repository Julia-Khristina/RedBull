...
| CONF — Confiabilidade       | O sistema deve manter consistência entre captura OCR, validação humana e persistência dos checkpoints | Taxa de inconsistência inferior a 1% entre dados capturados e dados persistidos durante a competição | Restrições `CHECK` em migrations (formato de `km`, `pace`, `tempo` e `status` — seção 3.6.3); tipagem em TypeScript dos models em `src/models`; validators de payload em `src/validators`; tratamento centralizado de erros via `errorHandler` + `AppError` (seg. 3.7); endpoint de OCR e de inconsistências planejados para a Sprint 5 |
| DES — Desempenho | O sistema deve atualizar o painel administrativo periodicamente durante a competição | Atualização concluída em até 5 minutos após novos checkpoints | Endpoints `GET /competitions/:competicaoId/ranking/teams` e `/athletes` implementados (api-documentation.html — seção 3.6.6); polling de 5 minutos previsto no front-end (Painéis da Sprint 4); testes de carga e medição de latência planejados para a Sprint 5 |
