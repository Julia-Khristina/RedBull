...
A escolha por uma página HTML versionada — em vez de uma tabela embutida diretamente no WAD — foi deliberada: permite uma visualização navegável e formatada (badges de status, blocos de código com destaque, links internos entre seções), mantém o versionamento alinhado com cada sprint de desenvolvimento e facilita a validação externa por parte de revisores, sem necessidade de clonar o repositório ou abrir uma ferramenta auxiliar como Swagger ou Postman.

A API segue convenções RESTful: recursos são organizados de forma hierárquica (por exemplo, `/competitions/:competicaoId/teams/:teamId/athletes`) e os métodos HTTP seguem semântica padrão (POST cria, GET consulta, PUT atualiza por substituição, PATCH atualiza parcialmente, DELETE remove). Os códigos de status seguem RFC 7231 (200, 201, 204, 400, 401, 403, 404, 409, 422, 500) e o tratamento de erros é centralizado em um middleware (`errorHandler`) que serializa as classes definidas em `src/errors/AppError.ts` em respostas JSON padronizadas. Cada endpoint listado na documentação está explicitamente rastreado a um ou mais Requisitos Funcionais (RF) e Regras de Negócio (RN) — a coluna RF/RN nas tabelas serve como ponte direta com as seções 3.1.1, 3.1.2 e 3.1.4, fechando o ciclo de rastreabilidade entregue na seção 3.9.
