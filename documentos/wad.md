...
A aplicação web é um sistema de gestão operacional especializado em competições esportivas de longa duração, com foco no formato Red Bull 24 Horas. O produto entrega um ambiente único para administradores e atletas, com interfaces dedicadas à operação de campo, à supervisão estratégica e ao acompanhamento público, todas conectadas a uma WebAPI centralizada (seção 3.7).

As funcionalidades **já entregues até a Sprint 3** são: cadastro completo de competições, equipes e atletas (CRUD); gestão de administradores; registro de checkpoints com vínculo obrigatório a corredor, competição, esteira e administrador responsável; consolidação de ranking de equipes e atletas por competição; identificação de cada equipe por UUID único para acesso público sem autenticação; e exportação de dados consolidados em formato JSON para auditoria pós-evento (seção 3.7).

Como funcionalidades **planejadas para as Sprints 4 e 5** estão: captura assistida por OCR a partir de fotografias dos visores das esteiras, atualmente em refinamento técnico; sinalização automática de inconsistências nos dados capturados; autenticação por token para rotas administrativas; e geração de relatórios analíticos com highlights de desempenho ao final da prova.

Os principais benefícios para o parceiro Red Bull são a redução do erro de apuração (meta declarada abaixo de 1%), a eliminação do retrabalho manual de transcrição, a visibilidade periódica do desempenho das equipes durante o evento e a rastreabilidade integral do método de cada checkpoint. Como diferenciais, a solução combina dedicação vertical ao formato de revezamento de ultra-resistência, validação híbrida entre OCR e operador e acesso público controlado via UUID — atributos ausentes em alternativas genéricas como planilhas ou formulários e em sistemas profissionais de cronometragem voltados a outras modalidades.
